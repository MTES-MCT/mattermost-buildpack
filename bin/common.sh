...
function configure_database() {
  ...
  case $db_type in
    postgres) 
      ...
      local PG_URL="postgres://${db_user}:${ENCODED_PASSWORD}@${db_host}:${db_port}/${db_name}?sslmode=${db_sslmode}"
...
function configure_mattermost() {
  ...
  jq ".EmailSettings.SendPushNotifications = \"false\"" "$MM_CONFIG" >"$MM_CONFIG.tmp" && mv "$MM_CONFIG.tmp" "$MM_CONFIG"
